    ...
    public:
        ...
        void update(SDL_Renderer* renderer, std::unique_ptr<AssetManager>& asset_manager, SDL_Rect& camera) {
            ...
            for (auto& entity : getSystemEntities()) {
                ...
                if (!flashlight.is_on) continue; // skip entirely if off
                ...
                float cx = (transform.position.x + flashlight.origin_offset_x) - camera.x;
                float cy = (transform.position.y + flashlight.origin_offset_y) - camera.y;
